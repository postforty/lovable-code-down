/**
 * Lovable Code Downloader - Reviewer Verification Test Suite (Round 1)
 */
...
function classifyTreeItem(el, label) {
  ...
  const treeItem = el && el.closest ? (el.closest("[role='treeitem'], [data-state], [aria-expanded], [data-item-type]") || el) : (el || {});
  ...
  // 5. Special exact files (extensionless or multi-part)
  const knownExactFiles = new Set([
    ...
    ".env.test", ".env.example", ".env.sample", ".env.staging", ".npmrc", ".nvmrc",
    ".yarnrc", "robots.txt", "humans.txt", "browserslist", ".babelrc",
    ".gitattributes", ".gitmodules", ".dockerignore", ".gitkeep", ".stylelintrc",
    ".stylelintignore", ".postcssrc", ".browserslistrc", ".lintstagedrc",
    ".eslintrc", ".prettierrc", ".commitlintrc"
  ]);
  ...
  // 7. Check file extension (e.g. accordion.tsx, vite.config.ts, project.json, vite-env.d.ts)
...
function runTests() {
  console.log("==================================================");
  console.log("🧪 SWE Light Reviewer Adversarial Test Suite (R1)");
  console.log("==================================================\n");
  ...
  // Test 2: Full 50+ File Hierarchy Path Reconstruction
  console.log("\n--- 2. Full 50+ File Hierarchy Path Reconstruction ---");
  ...
  // Test 3: Virtualized Scroll Window Simulation (Unmounted Top Rows)
  console.log("\n--- 3. Virtualized Scroll Sliding Window Simulation ---");
  // In a virtualized list, top rows unmount as you scroll down.
  // When scrolling monotonically from top to bottom, pathStack preserves active parent chain.
  let virtualPathStack = [];
  const collectedVirtualFiles = [];

  // Window 1 (Scroll position 0px): .lovable, project.json, public, favicon.ico, src, assets
  const window1 = [
    { label: ".lovable", level: 0 },
    { label: "project.json", level: 1 },
    { label: "public", level: 0 },
  ...
  // Window 2 (Scroll position 120px): .lovable & public unmounted! Visible: src/components/ui and items
  const window2 = [
    { label: "components", level: 1 },
    { label: "ui", level: 2 },
    { label: "accordion.tsx", level: 3 },
  ...
  // Window 3 (Scroll position 240px): ui unmounted! Visible: hooks, lib, routes
  const window3 = [
    { label: "hooks", level: 1 },
    { label: "use-mobile.tsx", level: 2 },
    { label: "lib", level: 1 },
  ...
  // Window 4 (Scroll position 360px): Root files visible
  const window4 = [
    { label: "package.json", level: 0 },
    { label: "vite.config.ts", level: 0 },
  ];
  ...
  // Test 4: Binary Asset / Data URL Packaging Logic
  console.log("\n--- 4. Binary Asset & Data URL Packaging Logic ---");
  ...
  const extractedBase64 = sampleDataUrl.split(";base64,")[1];
  assert(isDataUrl === true, "Detected data URL format");
  assert(extractedBase64 === "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNk+M9QDwADhgGAWjR9awAAAABJRU5ErkJggg==", "Extracted raw base64 payload");

  // Test 5: Safe State Cache Lookup (No Cross-Folder Basename Collisions)
  console.log("\n--- 5. Safe State Cache Lookup Test ---");
  const mockStateCache = {
    "button.tsx": "// Root button component",
    "src/components/ui/button.tsx": "// UI button component",
    "src/components/custom/button.tsx": "// Custom button component",
  };

  // When visiting src/components/custom/button.tsx (pathStack has src, components, custom)
  const currentPathStack = [{ level: 0, name: "src" }, { level: 1, name: "components" }, { level: 2, name: "custom" }];
  const targetLabel = "button.tsx";
  const targetFullPath = "src/components/custom/button.tsx";

  const resolvedCode =
    mockStateCache[targetFullPath] ||
    mockStateCache[`/${targetFullPath}`] ||
    (currentPathStack.length === 0 ? mockStateCache[targetLabel] : undefined);

  assert(resolvedCode === "// Custom button component", "Correctly resolved exact fullPath without collision with root button.tsx");
